...
function DiagramaCavalo({ pneus }: { pneus: any[] }) {
  const get = (n: number) => pneus.find((p: any) => p.tire_num === n)?.psi ?? null
  return (
...
export default function QRPage({ params }: { params: { token: string } }) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (json.ok && json.conjuntos?.length > 0) {
        const token = params.token
        const found = json.conjuntos.find((c: any) =>
          c.id === token ||
          c.device_sn === token ||
          String(c.id) === token
        ) || json.conjuntos[0]
      ...
      <nav style={{ background: '#141414', borderBottom: '0.5px solid #2a2a2a', padding: '0 16px', height: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {veiculo && (
          <span style={{
            ...
            {veiculo.online ? '● Online' : '○ Aguardando chip'}
          </span>
        )}
      </nav>
        ...
        {loading ? (
          ...
          <>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: '#444', letterSpacing: 1, marginBottom: 4 }}>CAVALO MECÂNICO</div>
              <div style={{ fontSize: 22, fontWeight: 500, letterSpacing: 2 }}>
                {veiculo.placa && !veiculo.placa.includes('æ') ? veiculo.placa : `Veículo #${veiculo.id}`}
              </div>
              <div style={{ fontSize: 11, color: '#555', marginTop: 2 }}>
                SN: {veiculo.device_sn} · {veiculo.pneus?.length || 0} pneus · {time}
              </div>
            </div>
            ...
            {veiculo.hasAlert && veiculo.alertas?.map((a: any, i: number) => (
              <div key={i} style={{ background: '#1a0000', border: '0.5px solid #7a2020', borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 500, color: '#FF6666', marginBottom: 2 }}>⚠ AÇÃO IMEDIATA</div>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#FF4444' }}>Pressão crítica — {a.location}</div>
                <div style={{ fontSize: 24, fontWeight: 500, color: '#FF4444' }}>{a.value} <span style={{ fontSize: 12, color: '#FF6666' }}>PSI</span></div>
            ...
            <div style={{ background: veiculo.hasAlert ? '#0c1a0c' : '#141414', border: `0.5px solid ${veiculo.hasAlert ? '#1e3a1e' : '#2a2a2a'}`, borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
              {veiculo.hasAlert ? (
                <>
                  <div style={{ fontSize: 11, fontWeight: 500, color: G, marginBottom: 8 }}>⚡ Agente VEPREX</div>
                  <div style={{ fontSize: 12, color: '#d0d0cc', lineHeight: 1.5, marginBottom: 6 }}>Pressão crítica detectada. Evite continuar rodando nessa condição.</div>
                  <div style={{ fontSize: 12, color: G, fontWeight: 500 }}>→ Calibrar na próxima parada segura.</div>
                </>
              ) : (
                <>
                  <div style={{ fontSize: 12, color: G, fontWeight: 500, marginBottom: 4 }}>✓ Todos os pneus dentro da faixa ideal</div>
                  <div style={{ fontSize: 11, color: '#555' }}>
                    {veiculo.online ? 'Monitoramento ativo. Próxima leitura em 30s.' : 'Instale o chip SIM M2M para ativar o monitoramento em tempo real.'}
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{ fontSize: 10, color: '#333', textAlign: 'center', padding: '8px 0' }}>
              Atualizado em tempo real · veprex.vercel.app
            </div>
          </>
        )}
      </div>
