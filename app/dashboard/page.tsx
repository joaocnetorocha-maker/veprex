...
const ALERT_TYPES: Record<string, AlertType> = {
  ...
  fast_loss: {
    id: 'fast_loss', severity: 'critical', emoji: '🔴',
    title: 'Perda rápida de pressão',
    urgencyLabel: 'AÇÃO IMEDIATA NECESSÁRIA',
    explanation: 'Foi detectada perda rápida de pressão em um dos pneus. Há indícios de vazamento ou dano. Priorize esta correção imediatamente.',
    impacts: [
      'pode comprometer o controle direcional do veículo',
      'há maior risco de perda de controle em velocidade elevada',
      'há indícios de vazamento ou dano no pneu',
    ],
    recommendation: 'Pare imediatamente em local seguro. Não continue a viagem antes de inspecionar o pneu.',
    messageTemplate: (cavalo, carreta, value, location) =>
      `🚨 VEPREX — URGENTE\nVeículo: ${cavalo}\nCarreta: ${carreta}\n\nProblema detectado:\nPerda rápida de pressão no pneu ${location}.\nPressão atual: ${value} PSI.\n\nPare imediatamente em local seguro e realize inspeção antes de continuar.`,
  },
}
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#1a1a1a', border: '0.5px solid #2a2a2a', borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 600, padding: '24px 24px 36px' }}>
        ...
        <textarea
          ...
          style={{ width: '100%', background: '#141414', border: '0.5px solid #2a2a2a', borderRadius: 8, padding: 14, fontSize: 13, color: '#d0d0cc', lineHeight: 1.6, resize: 'vertical', minHeight: 180, boxSizing: 'border-box', fontFamily: 'system-ui, sans-serif' }}
        />
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          ...
          <button onClick={handleSend} style={{ flex: 2, background: '#1a2e00', border: `0.5px solid ${G}`, borderRadius: 8, padding: '11px 0', fontSize: 13, color: G, fontWeight: 500, cursor: 'pointer' }}>
            {sent ? '✓ Enviado' : 'Enviar via WhatsApp'}
          </button>
        </div>
      </div>
    </div>
...
function AgenteVeprex({
cat > /Users/joaoprado/veprex/app/dashboard/page.tsx << 'ENDOFFILE'
'use client'
import { useState } from 'react'
    ...
    <>
      ...
      <div style={{ margin: '0 14px 12px', background: '#0c1a0c', border: '0.5px solid #1e3a1e', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ background: '#0f200f', padding: '10px 14px', borderBottom: '0.5px solid #1e3a1e', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ marginLeft: 6, display: 'flex', alignItems: 'center', gap: 4, background: '#3a0000', border: '0.5px solid #7a2020', color: '#FF6666', fontSize: 10, fontWeight: 500, borderRadius: 4, padding: '2px 8px' }}>
            🔴 Prioridade Alta
          </span>
        </div>
